...
// Funkcje pomocnicze dla intencji użytkowników
export const findUserIntentionById = async (intentionId: string) => {
  return await prisma.userIntention.findUnique({ 
...
export const validateIntentionText = (text: any): string | null => {
  if (!text || typeof text !== 'string' || text.trim() === '') {
    return 'Treść intencji (text) jest wymagana i nie może być pusta.';
  }
  return null;
};

export const validateIntentionOwnership = (intention: any, userId: string): boolean => {
  return intention && intention.authorId === userId;
};

export const checkRoseAccessForSharing = async (userId: string, roseId: string, userRole: any) => {
  const membership = await prisma.roseMembership.findFirst({
    where: { userId, roseId }
  });
  
  const rose = await prisma.rose.findFirst({
    where: { id: roseId, zelatorId: userId }
  });

  return membership || userRole === 'ADMIN' || rose;
};

export const validateSharingParameters = (isSharedWithRose: any, sharedWithRoseId: any): string | null => {
  if (Boolean(isSharedWithRose) && !sharedWithRoseId) {
    return 'Jeśli isSharedWithRose jest true, sharedWithRoseId jest wymagane.';
  }
  return null;
};

export const checkRosePermissionForIntentions = async (userId: string, roseId: string, userRole: any) => {
  const membership = await prisma.roseMembership.findFirst({
    where: { userId, roseId }
  });
